...
public function create($imagen,$mini,$titulo,$intro,$texto,
	                   $categoria,$fecha ,$usuario)
	{
		try
		{
			$stmt = $this->db->prepare("INSERT INTO noticias(imagen,mini,titulo,intro,texto,
													categoria ,fecha,usuario)
			 										VALUES(:imagen,:mini,:titulo,:intro, :texto,
			 										:categoria,:fecha, :usuario )");
...
public function getID($id)
	{
		$stmt = $this->db->prepare("SELECT * FROM noticias WHERE id=:id");
...
public function update($id,$titulo,$intro,$texto,$categoria ,$fecha,$usuario)
	{
		try
		{
			$stmt=$this->db->prepare("UPDATE noticias SET titulo=:titulo, 
		                                               intro=:intro, 
													   texto =:texto, 
													   categoria=:categoria,
													   fecha=:fecha,
	...
	public function dataview($query)
	{
		...
		if($stmt->rowCount()>0)
		{
			while($row=$stmt->fetch(PDO::FETCH_ASSOC))
			{
                ...
                <td align="center">
                <a href="id=<?php print($row['id']); ?> "   title="Editar" data-toggle="modal" data-target="#ModalEditSlider"><i class="glyphicon glyphicon-pencil"></i></a>
                </td>
                <td align="center">
                <a href="id=<?php print($row['id']); ?>" title="Eliminar" data-toggle="modal" data-target="#ModalDeleteSlider"><i class="glyphicon glyphicon-remove"></i></a>
                </td>
                </tr>
                <?php
			}
